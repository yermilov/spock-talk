...
But the same as with Gradle, where you do not need to no Groovy to develop build scripts, here you need to know almost nothing about Groovy to use Spock.
...
It always a change that for your 1% chance test requirement there is a Spock feature that makes it easier and more pleasant to develop.
...
After understanding its main concepts, you can test both Groovy and Java code, using either Groovy or pure Java expressions inside test blocks.
...
Being not so beautiful, it will work in 90% of the cases, when Spock is just a fresh look on the traditional approaches or when it introduces features that can be repeated.
...
Now Spock is public link:https://github.com/spockframework/spock[Github project, window="_blank"], developing very slowly after the last 1.0 release.
...
Obviously, the simplest case is when you are about to start new project.
In this case, just use Spock from the very beginning and enjoy it.
If you have legacy code without unit testing coverage, the case is pretty similar.
It's not a problem to start using Spock for it immediately.
...
As Spock is in some one just a JUnit runner both test suites are integrated into one view without any efforts.
In the following process, you can start continuous migration from JUnit to Spock, which is very easy as renaming test classes and changing ten files of code for each.
...
If you are about to introduce functional testing into your web project, Geb will be the best choise, that besides all other benefits, will introduce Spock to your project.

== Looking Further

If you do like or don't like this article, please share your opinion.
I will appreciate your feedback (especially negative one) in any form available for me attention (like social media, direct e-mail, or etc.).
...
- link:http://codepipes.com/presentations/spock-vs-junit.pdf[Spock vs JUnit by Kostis Kapelonis and Athens Greece, window="_blank"]
